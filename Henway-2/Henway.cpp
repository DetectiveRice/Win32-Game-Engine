...
void GameCycle()
{
	if (!_bGameOver)
	{
		...
		if (rand() % 100 == 0)
			if (rand() % 2 == 0)
				PlaySound((LPCSTR)IDW_CARHORN1, _hInstance, SND_ASYNC |
					SND_RESOURCE);
			else
				PlaySound((LPCSTR)IDW_CARHORN2, _hInstance, SND_ASYNC|
					SND_RESOURCE);
...
BOOL SpriteCollision(Sprite* pSpriteHitter, Sprite* pSpriteHittee)
{
	...
	if (pSpriteHittee == _pChickenSprite)
	{
		...
		if (--_iNumLives > 0)
			...
			PlaySound((LPCSTR)IDW_SQUISH, _hInstance, SND_ASYNC |
				SND_RESOURCE);
...
void MoveChicken(int iXDistance, int iYDistance)
{
	...
	if (_pChickenSprite->GetPosition().left > 400)
	{
		...
		PlaySound((LPCSTR)IDW_CELEBRATE, _hInstance, SND_ASYNC |
				SND_RESOURCE);
