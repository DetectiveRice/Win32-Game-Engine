...
BOOL Bitmap::Create(HDC hDC, LPTSTR szFileName)
{
	...
	if (pBitmapInfo != NULL)
	{
		...
		m_iHeight = (int)pBitmapInfo->bmiHeader.biWidth;
		
		//Skip (forward or bakcward to the color info, if necessary
		...
		//Geta handle to the bitmap and copy the image bits
		PBYTE pBitmapBits;
...
BOOL Bitmap::Create(HDC hDC, int iWidth, int iHeight, COLORREF crColor)
{
	...
	//Select the bitmap into the device conext
